...
export function renderEditorial(site,images){
 ...
 const spreads=site.layout.spreads.map((ids,i)=>`<div class="spread-band band-${i+1}"><div class="spread spread-${i+1}${film[i]?' motion-spread':''}">${ids.map((id,j)=>`<figure class="editorial-image image-${j+1}">${picture(id,{sizes:j===1?'(max-width:767px) calc(100vw - 40px), 40vw':'(max-width:767px) calc(100vw - 40px), 30vw'})}</figure>`).join('')}${film[i]??''}</div></div>`);
