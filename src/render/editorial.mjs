...
export function renderEditorial(site,images){
 ...
 const film=site.films.map((f,index)=>{
  ...
  return `<div class="film-band film-band-${index+1}"><div class="film-inner"><div class="film-copy"><p class="eyebrow">Swing × John Montgomery</p><p class="film-title">The Sunday<br> Edit</p><p class="film-intro">${e(c.intro)}</p></div><figure class="film-frame" data-film><video data-autoplay controls playsinline muted loop preload="none" width="${e(f.width)}" height="${e(f.height)}" poster="${url(f.posterSrc)}" src="${url(f.src)}" aria-label="The Sunday Edit campaign film">${f.captionSrc?`<track kind="captions" src="${url(f.captionSrc)}" srclang="en" label="English" default>`:''}</video><img data-film-fallback hidden src="${url(f.posterSrc)}" width="${e(f.width)}" height="${e(f.height)}" alt="${e(f.posterAlt)}" loading="lazy"><figcaption><span>The Sunday Edit — campaign film</span><span role="status" aria-live="polite"></span></figcaption></figure></div></div>`;
 });
 const spreads=site.layout.spreads.map((ids,i)=>`<div class="spread-band band-${i+1}"><div class="spread spread-${i+1}">${ids.map((id,j)=>`<figure class="editorial-image image-${j+1}">${picture(id,{sizes:j===1?'(max-width:767px) calc(100vw - 40px), 40vw':'(max-width:767px) calc(100vw - 40px), 30vw'})}<figcaption><span>${String(i*3+j+1).padStart(2,'0')}</span><span>${j===0?'The Sunday Edit':''}</span></figcaption></figure>`).join('')}</div></div>${film[i]??''}`).join('');
